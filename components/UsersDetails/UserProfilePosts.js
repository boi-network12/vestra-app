...
export default function UserProfilePosts({
  ...
  const renderMedia = (mediaItems) => {
    ...
    const images = mediaItems.filter(
      (item) => item && (item.type === 'image' || item.url.match(/\.(jpg|jpeg|png|gif)$/i))
    );
    const videos = mediaItems.filter(
      (item) => item && (item.type === 'video' || item.url.match(/\.(mp4|mov)$/i))
    );
  ...
  const renderQuotedMedia = (mediaItems) => {
    ...
    const isImage =
      firstMedia.type === 'image' || firstMedia.url.match(/\.(jpg|jpeg|png|gif)$/i);
    const isVideo =
      firstMedia.type === 'video' || firstMedia.url.match(/\.(mp4|mov)$/i);
    if (isImage) {
      return (
        <Image
      ...
      return <VideoPlaceholder uri={firstMedia.url} style={styles.quotedMedia} />;
    }
    return null;
  };
    ...
    >
      ...
      {item.quote && item.quote.user ? (
        ...
        <View style={[styles.repostContainer, { borderLeftColor: colors.primary }]}>
          <TouchableOpacity onPress={() => navigateToProfile(item.repost.user)}>
